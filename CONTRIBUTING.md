...
本项目以 **GPL-3.0** 发布。**提 PR 之前必须先接受 [CLA.md](CLA.md)** ——
它允许维护者把你的贡献用在闭源商业版本里。这一条写在最前面是故意的，
详见[第七节](#七cla提-pr-之前必须先接受)。
...
本项目使用 **CLA（贡献者许可协议）**，全文见 [CLA.md](CLA.md)。**请在动手写代码之前读它**，
不要等 PR 写完了才发现条款不能接受。

**为什么是 CLA 而不是 DCO** —— 本项目由单一作者开发，同时存在一个闭源商业版本
（本仓库是它的功能子集），两者共用同一份代码基。只用 DCO 的话，被合并的外部贡献
只以 GPL-3.0 授权给项目，维护者**无法**再把它放进闭源版；一旦发生，代码基就永久分叉。
CLA 是把这件事提前讲清楚，而不是等你的 PR 合并之后才发现自己的代码进了商业产品。

**它对你意味着什么**（完整条款以 [CLA.md](CLA.md) 为准）：

- 你**保留**自己贡献的著作权，不转让；
- 但你授权维护者可以**以任何许可证再许可**你的贡献，**包括闭源商业许可**；
- 也就是说，**你的代码可能出现在一个收费的闭源软件里，你不会因此获得报酬**。

不接受是完全合理的选择。开 issue、报缺陷、提文档反馈、fork 自己维护——都不需要签 CLA。

**怎么表示接受**：在 PR 描述里逐字写下这两行（PR 模板里有现成的位置）：
...
没有这句话的 PR 不会被合并，无论代码质量如何——这条链路一旦断了很难事后补齐。

代表公司/组织提交，或你的贡献属于职务作品的，**先走 [SECURITY.md](SECURITY.md) 的私密通道联系**，
不要直接提 PR（见 CLA.md 第七节）。

> 本项目**不再要求** `git commit -s`。提交签名（DCO）与 CLA 是两套东西，
> 同时要求只是徒增摩擦；接受与否以 PR 里那句声明为准。
