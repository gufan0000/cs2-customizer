...
PAGE_HELP_TEXTS = {
    ...
    "audio_import_wizard": (
        "<b>功能说明</b><br>"
        "把你下载或自己做的一堆音频文件，按目录结构认出来并放进软件的音效库。<br><br>"
        "<b>怎么用</b><br>"
        "1. 第 1 步选源目录 —— 选**解压后**那个文件夹，里面应该是按风格分的子目录<br>"
        "2. 点「扫描目录」。扫描**只看不写**，不会动你现有的音效库<br>"
        "3. 第 2 步看扫描结果：能认出来的、有冲突的、没认出来的各多少<br>"
        "4. 确认无误后点「开始导入（不覆盖已有文件）」<br>"
        "5. 没认出来的文件不是死路：用「把未识别音频导入为新风格…」手动归类<br><br>"
        "<b>注意事项</b><br>"
        "• 导入默认不覆盖同名文件，先扫描再导入是安全的<br>"
        "• 勾了「仅生成建议」就只出报告、一个文件都不复制"
    ),
    "audio_task_panel": (
        "<b>功能说明</b><br>"
