...
PAGE_HELP_TEXTS = {
    ...
    "gun_sound": (
        ...
        "手枪、冲锋枪、步枪、狙击枪、霰弹枪、机枪、宙斯电击枪都能换。"
        "全自动武器扫射时软件会多留一点原声（约 20%）当节奏骨架 —— 游戏每 0.1 秒左右才报一次开火，"
        "留着原声，两次上报之间的那几发不会听起来像断了。<br><br>"
        "<b>使用方法</b><br>"
        "1. 在本页状态卡最上面打开「总开关」<br>"
        "2. 进入对应武器选项卡，为每把武器单独选择「枪声风格」<br>"
        "3. 调整「原声保留」（原始枪声音量，默认 18%）和「静音覆盖时长」（压低原声的时长），用测试按钮试听<br>"
        "4. 「整套套用」按<b>套系</b>配：素材作者给每把枪起的名字不一样（离子AK、离子战神、离子大狙…），"
        "名字开头相同的算一套，选一套各枪各配各的；确认框会逐把列出谁配哪个<br><br>"
        "<b>调校建议</b><br>"
        "• 单发 / 慢射速武器（AWP、沙鹰）可适当增加「静音覆盖时长」，让自定义音更完整<br>"
        "• 手枪等快射速武器建议缩短覆盖时长，避免连续开火时声音糊成一片<br>"
        "• 全自动武器（AK / M4 / 冲锋枪 / 机枪）的素材选 1 秒以上、有余音的样本：前一发还在响时下一发叠上来，扫射才连贯；短促的单发样本会听出空洞<br>"
        "• 想保留原枪声质感，可调高「原声保留」音量<br><br>"
        "<b>自定义枪声目录</b><br>"
        "1. 每把武器的目录结构为 <code>AppData/Local/CS2Customizer/resources/audio/gun_sounds/武器名/风格名/</code><br>"
        "2. 风格文件夹里放 1 个或多个 MP3/WAV/OGG 文件：放多个时每一发随机换一个取样（不和上一发重复），"
        "扫射就不会一个声音复读；最多用前 5 个<br>"
    ...
    "special_sound": (
        ...
        "• 投掷物：丢出手雷 / 闪光弹 / 烟雾弹 / 燃烧瓶 / 燃烧弹 / 诱饵弹的那一刻<br>"
        ...
        "<b>自定义音效目录</b><br>"
        "所有目录都在 <code>AppData/Local/CS2Customizer/resources/audio/</code> 下。"
        "<b>除了回合音效，其余几类都不挑文件名</b> —— 风格目录里放一个音频文件就行，"
        "叫什么都认（支持 mp3 / wav / ogg）：<br>"
        "• 投掷物：<code>grenade_sounds/类型/风格名/</code>，"
        "<b>类型这一层只认这六个英文名</b>："
        "<code>hegrenade</code>（手雷）、<code>flashbang</code>（闪光弹）、"
        "<code>smoke</code>（烟雾弹）、<code>molotov</code>（燃烧瓶）、"
        "<code>incgrenade</code>（燃烧弹）、<code>decoy</code>（诱饵弹）。"
        "写成中文或别的拼法，软件读不到<br>"
        "• C4：<code>c4_sounds/风格名/</code>，"
        "文件名里带「拆除 / defuse」「爆炸 / explode」的分别作拆除音和爆炸音，"
        "其余那个作安放音<br>"
    ...
    # ⚠ RN-001b（批 34）：这一页原来**没有帮助文案** —— 而外审
    #   ④「你想知道这功能怎么用，这一屏上有地方问吗」**6/6 答「没有」**。
    #   两件最容易卡住人的事写在最前面：进游戏看不到（显示模式）、
    #   刷不出内容（没登录）。
    "fun_afterlife": (
        "<b>功能说明</b><br>"
        "在 CS2 里阵亡之后，在屏幕边上贴一块竖屏短视频窗口；复活时自动暂停并切回游戏。"
        "纯整活功能，默认关闭。<br><br>"
        "<b>先做这两件事</b><br>"
        "1. 去 CS2 把显示模式改成「<b>无边框窗口化</b>」——独占全屏会把贴屏窗口整个盖住，"
        "什么都看不到，切换时还会黑屏几秒<br>"
        "2. 点「打开并登录抖音」<b>先登录一次</b>——没登录刷不出内容<br><br>"
        "<b>使用方法</b><br>"
        "1. 打开总开关（和「基础设置」里那颗是同一颗）<br>"
        "2. 在「触发条件」里勾上你实际会玩的对局模式——"
        "<b>默认只勾了「死亡竞赛」和「休闲」，打排位的要自己勾上「竞技」</b><br>"
        "3. 「预览效果」会当场把窗口弹出来看位置，点「收回窗口」结束<br><br>"
        "<b>隐私</b><br>"
        "登录态保存在软件自己的浏览器数据目录里，与你日常使用的浏览器完全隔离；"
        "账号密码只在浏览器内输入，本软件不经手。<br><br>"
        "<b>收不回来怎么办</b><br>"
        "「高级 · 最长停留」是兜底：退出游戏等收不到复活信号的情况下，"
        "到点会自动把窗口收回来。"
    ),
    "music": (
        "<b>功能说明</b><br>"
    ...
    "preset_center": (
        ...
        "<b>怎么用</b><br>"
        # ⚠ 批 40：这三句原来说的是改版前那套动线（四颗按钮 + 「应用读入的预设包」）。
        #   RN-167 那条棘轮只拦「点名了一个不存在的控件」，而这里点的都是通名，
        #   所以它一句都不会红 —— ⭐ **一份不点控件名的说明书，改版之后不会报错，
        #   只会变成一段对不上号的话。**
        "1. 在「保存/导出的范围」勾上想打包的类别<br>"
        "2. 想换一整套体验时，用「内置精选」或「我的预设」，选一套点应用<br>"
        # ⚠⚠ 这一行上一版写的是「导出这一套，发给朋友」，而屏幕上那颗按钮叫
        #   「导出成文件发给朋友」——**全仓没有任何控件叫前者**。
        # ⭐⭐ RN-167 那条专防「说明书点名一个不存在的控件」的棘轮**照不到它**：
        #   它的 `CLICK_CONTEXT` 正则要求「动词 + 可选方位词 + 「名字」」，
        #   而这里名字出现在**句首**（「3. 「导出…」会生成一个文件」），
        #   引号前面没有动词 ⇒ 这一句压根不进分母。
        #   （批 39 那条「守卫用自己的白名单当分母」的第三个实例：
        #    这次分母是被**句式**筛掉的。）
        "3. 「导出成文件发给朋友」会生成一个文件；对方点「打开一份配置文件」"
        "或者直接把文件拖进预设中心，确认之后就套用了<br><br>"
        # ⚠ 「合并 / 覆盖」这一段原来讲的是页面上那个下拉框，而批 40（RN-484）
        #   把它整个撤了 —— 现在这个选择只在**确认框里**出现，
        #   而且只在这份文件真的会碰到那 5 个 dict 型键时才出现。
        #   ⭐ 说明书说的是"哪里有个什么"，控件搬了家，这段话就得跟着搬。
        "<b>打开别人的文件时，那两颗按钮</b><br>"
        "• 只有当对方那份文件里带着「你也自己加过条目」的那几类（HUD 规则、"
        "特殊音效、自定闪光、开镜放大）时，确认框才会问你一句<br>"
        "• 「留着，只补上文件里的」＝你自己加的条目保留；"
        "「清掉，只要文件里的」＝这几类只留对方的<br>"
        "• 其余类别（准心、屏幕特效、局内视角）两种做法结果完全一样，所以不会问<br>"
        "• 无论哪种，应用前都会自动存一份「软件设置快照」，随时能在快照页回滚<br><br>"
        "<b>按地图自动切换</b><br>"
    ...
    "config_snapshot": (
        "<b>功能说明</b><br>"
        "软件设置快照就是给你当前的全部设置「拍张照片」存下来，作为一个可回滚的安全点。改坏了、试了新配置不满意，随时能一键还原到之前某个快照。<br><br>"
        "<b>怎么用</b><br>"
    ...
    "audio_import_wizard": (
        "<b>功能说明</b><br>"
        "把你下载或自己做的素材认出来、摆进对应目录。"
        "压缩包、文件夹、单个素材文件都收，**不用先解压**。<br><br>"
        "<b>怎么用</b><br>"
        "1. 把压缩包 / 文件夹 / 单个素材文件**直接拖进页面**，"
        "或者点「选择文件夹」「选择压缩包 / 文件…」<br>"
        "2. 点「扫描素材」。扫描**只看不写**，不会动你现有的素材<br>"
        "3. 第 2 步看扫描结果：能认出来的、有冲突的、没认出来的各多少<br>"
        "4. 确认无误后点「开始导入（不覆盖已有文件）」<br>"
        "5. 认不出来的会**问你一次**，并把依据摆出来给你看<br>"
        "6. 导完之后提示条上有「撤销」，点一下就能全部收回<br><br>"
        "<b>注意事项</b><br>"
        "• 导入默认不覆盖同名文件，先扫描再导入是安全的<br>"
        "• 勾了「仅生成建议」就只出报告、一个文件都不复制<br>"
        "• 击杀音效/语音产品按 `1`~`5` 的文件名播放，"
        "名字不对的会**替你改好**并在提示条里说明<br>"
        "• RAR / 7z 打不开，但会认出来并告诉你先解压"
    ),
    "audio_task_panel": (
        "<b>功能说明</b><br>"
        "导入音效包、刷新资源、重新载入音频这些要跑一会儿的事，都在这里看进度和结果。<br><br>"
        "<b>怎么用</b><br>"
        "1. 任务不是在这里发起的 —— 你在别处点了导入或刷新，它才会出现在这里<br>"
        "2. 「当前状态」那排显示正在跑的那个；下面的历史是已经跑完的<br>"
        "3. 某次导入没生效时，先来这里看它到底跑没跑、成没成<br><br>"
        "<b>注意事项</b><br>"
        "• 历史只保留本次运行期间的记录<br>"
        "• 「没有任务在跑」是正常状态，不是出错"
    ),
    "audio_replay": (
        "<b>功能说明</b><br>"
        "回答「刚才那个音效为什么没响」。软件每次播音效都会记一笔：播了、被别的音效顶掉了、还是压根没找到文件。<br><br>"
        "<b>怎么用</b><br>"
        "1. 先去对应功能页试听一次（比如「击杀音效」），这里才会有记录<br>"
        "2. 回到本页点「刷新」，最近的记录会按时间倒序列出来<br>"
        "3. 用上面三个筛选缩小范围；「结果」那一列写着这次到底成没成<br>"
        "4. 想把记录发给别人排查，用「导出 JSON」<br><br>"
        "<b>注意事项</b><br>"
        "• 记录只在软件本次运行期间保留，重启就清空<br>"
        "• 「重放选中」是拿软件自己再放一次那个音效，不会改你的任何设置"
    ),
    "audio_health": (
        "<b>功能说明</b><br>"
        "资源体检会检查你的音效 / 图片等资源是否完整，帮你回答「为什么这个音效不响」。<br><br>"
        "<b>怎么用</b><br>"
        "1. 点「立即体检」，软件会扫描各功能的资源目录<br>"
        "2. 报告会列出：缺失的目录、配置里选了但文件不存在的风格、连杀档位不全的风格等<br>"
        "3. 对能自动处理的问题，可用「一键修复（只补不删）」补齐缺失目录 / 清理无效引用<br><br>"
        "<b>注意事项</b><br>"
        "• 体检是只读检查，不会删你的音频文件<br>"
        "• 若某功能没声音，先来这里体检，往往能直接看出是「没放文件」还是「选错风格」"
    ),
    # ⚠ RN-001b（批 45）：这一页原来**没有帮助文案** —— 而 `ui_help_panel` 的
    #   帮助按钮是按这张表有没有这一页的键来决定露不露的，于是 `about` 上
    #   连那颗「?」都不存在。⭐ 一份缺失的文案不会报错，它只是让一颗按钮消失。
    # ⭐ 内容按这一页**真实有的四张卡**写（产品信息 / 更新与渠道 / 开源与 Star /
    #   反馈与求助），不写这一页没有的东西 —— 这一页自己就是被
    #   RN-451「页头承诺了一个不存在的功能」咬过的那一族。
    "about": (
        "<b>功能说明</b><br>"
        "这一页是软件的「身份证 + 求助台」：看版本和作者、查更新、找源码、"
        "以及在出问题时把诊断信息一次性交给别人。<br><br>"
        "<b>四张卡各管什么</b><br>"
        "1. <b>产品信息</b> — 版本号、作者、群号；「三步上手引导」按钮会打开"
        "首次运行那套引导，用来确认 CS2 目录选对了<br>"
        "2. <b>更新与渠道</b> — 手动检查更新<br>"
        "3. <b>开源与 Star</b> — 开源版仓库地址（可复制）<br>"
        "4. <b>反馈与求助</b> — 复制诊断信息、下载解决包、复制群号<br><br>"
        "<b>注意事项</b><br>"
        "• 开源版是**功能子集**，不等于你正在用的这一份<br>"
        "• 「复制诊断信息」只收版本 / 系统 / 配置概况，"
        "不含账号口令，粘给别人之前可以自己先看一眼<br>"
        "• 这一页不改任何设置 —— 看完就走，不会留下任何改动"
    ),
}

# ⭐⭐ 账号页的帮助文案住在 `account_help.py` 里，**不在上面这张表里**。
#
# 它原来就在表里，靠 `oss_sync/patches/ui_help_panel_py.patch` 在同步时删掉那一段。
# 而那个补丁只做一件事（删这一段），其余全是上下文 —— 上游每改一次这张表附近的字，
# 补丁就整条打不上，同步被卡死（RN-510 收了三轮这个税）。
# ⇒ 照 `oss_sync/README.md` 那条规矩：把它改成**结构上可选**（独立模块 + 能力检查），
#   然后把补丁删掉。开源版的排除清单里已经有整条账号链路，这个文件跟着一起不进去。
# ⭐ **补丁集应该随时间趋近于零** —— 这是第一笔真正减下去的。
try:
    from account_help import ACCOUNT_HELP_TEXT
except ImportError:                       # 开源版不带账号链路
    pass
else:
    PAGE_HELP_TEXTS["account"] = ACCOUNT_HELP_TEXT
