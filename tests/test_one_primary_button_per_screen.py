...
#:   · `utility`（2026-08-31 批 36 加入，RN-303/RN-452）：底栏那**五个位置，
#:     五个都是卡内某颗按钮的第二份**（全站 36 处里这一页独占 5 处，最多的一页）
#:     ⇒ 全撤，主按钮位随之空着。
#:     ⭐ **不给它随便提一颗当主按钮**：这一页卡内那几颗全是维护动作
#:     （开文件夹 / 刷新 / 预览），把其中任何一颗染紫都会重演 `magnifier` 那件事 ——
#:     **让全页最响的按钮指向一个不是目标的动作**。
#:     真正的「第一步」是状态卡里那颗总开关，而它本来就在第一屏最上面（外审 ① 6/6 答对）。
#:   · `preset_center`（2026-09-01 批 38 加入，RN-281/RN-452）：底栏主按钮位原来放的是
#:     「应用当前预设」，而在**每个人打开这一页看到的那个状态下**，
#:     它写回去的正是 `export_bundle(当前 config)` —— 实测 57 个键**0 个**会变，
#:     然后弹一句「已应用类型: hud_rules, screen_effects, …」。
#:     ⭐⭐ 外审行为题 ①「你最先会点哪个控件」**12/12 全指它** ——
#:       **全页最响的那颗按钮，做的是「什么都不做」，并且报告它做成了。**
#:       （`magnifier` 那次是「做了离目标最远的那件事」，这次是更纯的形态。）
#:     它只有一个真对象：刚用「导入预设包」读进来、还没落地的那一份 ⇒
#:     动作留在工作台卡内那颗（改名「应用读入的预设包」、没对象时禁用 + tooltip），
#:     底栏主按钮位随之空着。
