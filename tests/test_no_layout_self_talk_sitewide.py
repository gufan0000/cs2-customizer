...
#: ⚠⚠ **2026-09-01 批 37 补第三条，而且是我自己犯出来的。**
#: 给 `account` 补「不想登录的人下一步去哪」，我写了
#:   「还没登录。**左边那些功能**（准心、音效、开镜放大…）本来就不需要账号，
#:    不想登录就直接去用。」
#: 改完复跑，紧凑档 3/3 判高，逐字写「文案提示『去用左边那些功能』，
#: 但侧边栏已折叠隐藏，玩家看不到且找不到入口」—— 与上面 docstring 里
#: `flash` 那次**一模一样的翻车**，隔了 20 多批又来一次，犯的人还是我。
#:
#: 而上面那两条**没抓到它**：它指的是导航，却**没说出"栏/导航/菜单"这几个字** ——
#: 用「那些功能」代指。⇒ 补的正是这个形态：**按位置指一堆没名字的东西**。
#: ⭐⭐ 我一度想把它推广成 `(左|右|上|下|顶|底)(边|侧|方|面|栏)` 一把抓 ——
#:   **而上面那段注释里逐字写着这个做法已经被实测否决过**（一刀切量出 24 处，
#:   其中 20 处是对的：OSD 位置的选项值、两档都在的「右上角 ? 按钮」…）。
#:   ⇒ 我差点用一条被否决过的方案，去修一个它当年就说清了原因的问题。
#:   ⭐ **在自己要改的文件里，先把已经写下的结论读完。**
DIRECTION_PATTERNS = [
    r"(左侧|左边|左方|右侧|右边)[^，。；]{0,6}(页面|页|栏|导航|菜单)",
    r"侧栏",
    r"(左侧|左边|左方|右侧|右边)(那些|这些|那几个|这几个|那一堆)",
]
...
def test_the_direction_patterns_catch_the_sentences_i_actually_wrote():
    """⭐ 阳性对照：那三条模式必须真的咬得动**历史上真出事的那几句**。

    上一条判据今天是绿的（0 处）。而一条恒绿的否定断言，和一条什么都不匹的正则，
    在测试报告里长得一模一样 —— 本仓已经为这件事付过好几次账。
    ⇒ 这里直接喂进**真的写出来过、且真的被外审判高过**的那几句，逐句证明它红。
    """
    real_mistakes = [
        # RN-075 那轮（见上面 docstring）：紧凑档 3/3 判高
        "先在左侧「基础设置」页的「功能开关」里打开「自定闪光」",
        # 批 37（本批）：给「不登录的人」指路时写的，紧凑档 3/3 判高
        "还没登录。左边那些功能（准心、音效、开镜放大…）本来就不需要账号，不想登录就直接去用。",
        "导航侧栏里能找到",
    ]
    for sentence in real_mistakes:
        assert any(re.search(p, sentence) for p in DIRECTION_PATTERNS), (
            f"这句话历史上真的翻过车，而模式抓不到它：{sentence!r}")

    # ⛔ 反面守卫：**不许一刀切**。下面这几句里的方位词是对的
    #   （OSD 位置的选项值 / 两档都在的控件），而一刀切正是这条判据被否决过的第一版：
    #   它量出 24 处，其中 20 处是这类。⭐ 判据一旦开始冤枉正确的文案，就会被人关掉。
    legitimate = [
        "右下角",
        "屏幕左侧",
        "点右上角「?」看用法",
    ...
    for sentence in legitimate:
        assert not any(re.search(p, sentence) for p in DIRECTION_PATTERNS), (
            f"这句话里的方位词是对的，却被判红了：{sentence!r}")
