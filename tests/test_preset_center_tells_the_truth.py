...
def test_the_way_to_open_a_friends_file_is_on_the_first_screen(qapp, real_window, music_bar):
    ...
    # 批 52 实测 1175（撤掉状态卡之后）→ ⭐ **批 67 RN-551 之后 1089，矮了 86px**：
    # 高度令牌原来被当成 QSS 的内容盒发出去，全站控件比规格高 16~18px；
    # 单位修对之后这一页的按钮/下拉框各矮一截，内容整体收了 86px。
    # ⭐ 这个数**不是我调出来的**，是这条守卫逼出来的 —— 它正是 RN-551 在这一页的收益。
    _CONTENT_H = 1089
