...
def test_a_batch_row_only_claims_closures_the_registry_agrees_with():
    ...
    #: ⭐⭐⭐ **这道守卫自己把「有一批账没有状态」当成了永远成立的前提。**
    #:   它原来断言 `len(on_file) > len(status)` —— 那个不等号成立的**唯一原因**
    #:   就是旧账逐页表没有状态列。2026-09-02 批 41 给那张表补上状态列之后，
    #:   两个数变成相等，这道守卫**红在一件好事上**。
    #: ⭐⭐ 一道空转守卫，如果拿「某个盲区还在」当自己的存在性证据，
    #:   那它就会在盲区被补上的那天报警，而那天恰恰是最不该报警的一天。
    #: ⇒ 换成问它真正想确认的事：**这两个分母都真的读到了东西**。
    # ⚠ 这两句**故意分开写，不写成 `a >= b >= 300` 的链式比较**：
    #   `test_judges_are_not_idling` 的守卫识别器只认**单个操作符**的比较
    #   （`len(t.ops) == 1`），链式写法有两个操作符 ⇒ 它认不出这是一道分母守卫，
    #   于是这条判据会被记进「空转风险」清单。实测：写成链式当场把那个数从 77 顶到 78。
    ...
    assert len(on_file) >= len(status), (
        f"在册 {len(on_file)} 条 / 有状态格 {len(status)} 条 —— 解析器多半没读到表，"
        "这条判据的存在性检查已经瞎了。"
    )
