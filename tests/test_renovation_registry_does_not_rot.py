...
STATUS_WORDS = (
    ...
    "并入",                              # ↑ 结了
    "已裁", "立案",                       # ↑ 没结（已裁 = 批了但还没做）
    # ⭐ 2026-09-16 批 98：「新立」从这里删掉了 —— RN-651 是最后一个用它的条目，
    #   批 98 把开着的账全部结清后它就成了古董（同「待裁定」「未判定」，双向断言逼出来的）。
    #   删掉不留洞：`_is_open()` 认不出的一律判没结；有人再写「新立」那一刻
    #   `test_every_status_cell_starts_with_a_declared_word` 会红，逼着加回来。
