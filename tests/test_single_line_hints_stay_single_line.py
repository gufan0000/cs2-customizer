...
SINGLE_LINE_HINTS = (
    ...
    #   表空掉时整条判据会静默地什么都不测。
    # ⚠ 2026-08-29（RN-405②·批 25）：这句文案改了（原来是「还没有任何风格，点右边的
    #   「＋ 导入」装一套。」，退成了纯说明，因为同一屏上页头已经在下达"第一步"指令）。
    # ⭐⭐ **这一格是被回退验证的基线拦下来的，而它拦得住的原因是上面那条分母守卫**：
    #   判据找不到那个标签时**报红**，不是绿着跳过。
    #   ⚠ 对照批 24 那两条失效锚点 —— 那两条改完文案之后**照样是绿的**，
    #     要靠专门的失效体检才查得出来。
    #   ⭐ **一条判据的锚点腐烂时会不会喊疼，取决于它有没有替自己写一条分母守卫。**
    ("kill_icon", "装好的图标包会排在这一行",
     "空状态说明，横排里独占一行；折行时它只拿到 232px 而同排空着 700 多 px"),
)
