...
2. 环境变量 FANPAI_DEBUG_LOG 与 config.json 的 debug_file_log 都能开回 DEBUG，
   排障能力不丢。
...
def _clear_env(monkeypatch):
    monkeypatch.delenv("FANPAI_DEBUG_LOG", raising=False)
    yield


def test_defaults_to_info(monkeypatch, tmp_path):
    """没有任何开关时,文件日志走 INFO。"""
    monkeypatch.setenv("FANPAI_CONFIG_DIR", str(tmp_path))
...
def test_env_var_enables_debug(monkeypatch, tmp_path, value):
    monkeypatch.setenv("FANPAI_CONFIG_DIR", str(tmp_path))
    monkeypatch.setenv("FANPAI_DEBUG_LOG", value)
...
def test_config_field_persists():
    ...
    conftest 已把 FANPAI_CONFIG_DIR 指到临时目录，不会碰用户真实配置。
    """
...
def test_purge_removes_only_expired(tmp_path):
    _touch(tmp_path / "fanpai_20250101.log", 30)        # 过期
    _touch(tmp_path / "fanpai_20250102.log.1", 20)      # 过期(轮转副本)
    _touch(tmp_path / "bootstrap_crash_20250101.log", 40)  # 过期
    _touch(tmp_path / "fanpai_20260807.log", 1)         # 新的,必须留
    ...
    assert names == {"fanpai_20260807.log", "native_crash.log", "别人的文件.txt"}
...
def test_purge_survives_locked_file(tmp_path, monkeypatch):
    ...
    _touch(tmp_path / "fanpai_20250101.log", 30)
    _touch(tmp_path / "fanpai_20250102.log", 30)
...
def test_log_dir_is_isolated_in_tests():
    ...
    assert os.environ.get("FANPAI_LOG_DIR"), "conftest 应当设置 FANPAI_LOG_DIR"
    assert resolved == os.environ["FANPAI_LOG_DIR"]
    if real_appdata:
        assert not resolved.startswith(os.path.join(real_appdata, "FanTool", "logs"))


def test_purge_never_runs_under_pytest():
    """双保险：即便忘了设 FANPAI_LOG_DIR，测试进程也不许触发清理。"""
...
def test_audit_detects_info_level_logs():
    ...
    info_lines = ["[2026-08-07 10:00:00] [INFO] [FanPai:1] [日志] 文件日志级别 = INFO（如需音频事件审计请开启 debug_file_log）"]
    debug_lines = ["[2026-08-07 10:00:00] [INFO] [FanPai:1] [日志] 文件日志级别 = DEBUG（排障模式）"]
...
def test_runtime_log_dir_honors_env_override(monkeypatch, tmp_path):
    ...
    R4 实测踩到：logger 认 `FANPAI_LOG_DIR`、main_widget 的 faulthandler 目录
    解析不认，于是 `scripts/live_run.py` 声称"隔离运行"，实际每跑一次都往用户
    真实的 `%LOCALAPPDATA%\FanTool\logs\native_crash.log` 追加会话记录。
    诊断文件写错地方比不写更糟——它污染的是真实崩溃的取证材料。
    """
    import main_widget

    monkeypatch.setenv("FANPAI_LOG_DIR", str(tmp_path))
...
def test_runtime_log_dir_falls_back_to_appdata(monkeypatch):
    ...
    monkeypatch.delenv("FANPAI_LOG_DIR", raising=False)
