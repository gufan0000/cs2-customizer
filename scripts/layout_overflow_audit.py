...
KNOWN_COMPACT_DEBT: dict[tuple[str, str], tuple[int, str]] = {
    ...
    #   一行 ~800px 顶穿整页最小宽。改成 FlowLayout（放不下就换行）之后
    #   横向溢出 **29px → 0**。
...
KNOWN_EXPANDED_DEBT_COMPACT: dict[tuple[str, str], tuple[int, str]] = {
    ...
    # ⚖ 2026-09-24 RN-681：那句前提拆成「放大的动作行 + 后果行」，展开档再多 29px（40 → 69）；默认档仍 rc=0。
    ("flash", "clip"): (69, "状态卡补了独占全屏前提（动作行放大）；只在控制条展开 + 紧凑档出现"),
}
...
KNOWN_SQUASHED_DEBT: dict[tuple[str, str], tuple[int, str]] = {
    #: ⚠ 2026-09-16 批 98（RN-651）：下面 8 条只在**开源版**命中 —— `resources/` 不进开源仓，
    #:   开源版没有内置字体，回落字体的度量把这些按钮多挤 2px（公开仓 CI 与本机开源树都报，
    #:   闭源树 rc=0）。它们和上面那族「字号放大档差 2px」是同一件事，只是 M6 之后才出现的键。
    #:   闭源版这些键不命中 ⇒ 只提醒不红（上限语义）。
    ("about", "打开项目仓库"): (2, "开源版回落字体差 2px（about 是开源版自有实现）"),
    ("about", "提交 Issue"): (2, "开源版回落字体差 2px（about 是开源版自有实现）"),
    ("death_sound", "新建风格"): (2, "开源版回落字体差 2px"),
    ("kill_icon", "导入图标包…"): (2, "开源版回落字体差 2px"),
    ("kill_sound", "风格工具"): (2, "开源版回落字体差 2px"),
    ("kill_voice", "风格工具"): (2, "开源版回落字体差 2px"),
    ("reload_sound", "新建风格"): (2, "开源版回落字体差 2px"),
    ("switch_weapon", "新建风格"): (2, "开源版回落字体差 2px"),
