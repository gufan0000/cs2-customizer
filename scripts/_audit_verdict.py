...
def announce(name: str, rc: int) -> None:
    """只**打出**裁定行，不退出 —— 给「打完还得跑收尾」的脚本用。

    `deliver()` 打完就 `os._exit`，适合算完即走的审计。
    但有的门禁跑完还必须走一段收尾（`build_search_index.py` 要拆掉真窗口、
    还原沙箱），而**那段路正是退出码被洗掉的地方**：

    ⚠ RN-511 实测（2026-09-04）：CI 上「索引与代码同步。」打完、
    「退出清理完成，共 18 步」也打完，进程退出码却是 **-1073740791（0xC0000409）**。
    本机同一条命令连跑两遍，管道那遍 0、重定向那遍 -1073740791 ⇒ **它是随机的**。

    ⇒ 这类脚本要在**判定那一刻**就把裁定落进日志，然后照常收尾。
      收尾崩了也改不掉已经打出去的那一行；而没走到判定就死掉的，
      日志里一行裁定都没有 ⇒ `verdict.ps1` 按失败处理。**洗不成假绿。**
    """
    print(f"{VERDICT_PREFIX} {name} rc={int(rc)}", flush=True)
