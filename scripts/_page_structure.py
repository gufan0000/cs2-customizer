...
#: ⚠⚠ 2026-09-10 批 74（RN-527①）：这张表**逐字写死了当时那几句文案**，
#: 于是我一改芯片措辞（「音频 · 检查」→「音频 · 需检查 N 项」），
#: 它当场认不出来、把**这台机器上的数**写进了基线 ——
#: ⭐⭐⭐ **一张按文案字面匹配的脱敏表，会在文案改动的那一刻悄悄失效，
#:   而失效的样子是「基线里多了一个环境事实」，不是「有人报错」。**
#: ⇒ 这几条改成**只认前缀**（「音频 · 」），别再把后半句写进正则；
#:   后半句正是最会改的那一半。
#: ⚠ 代价是「后半句写成什么」从此不由基线看着了 —— 那不是放过它，是**换个人看**：
#:   `tests/test_the_health_chips_say_what_they_mean.py` 拿合成报告钉住每一种状态下
#:   芯片的**逐字文案**（那是纯函数，与这台机器无关）。
#: ⭐ **基线管「结构变没变」，那支文案判据管「话说得对不对」** —— 一个环境相关、
#:   一个确定性，混在一条里必然要牺牲其中一个。
_ENV_DEPENDENT_TEXT = (
    (re.compile(r"^当前权限："), "当前权限：<环境相关>"),
    (re.compile(r"^体检 · "), "体检 · <环境相关>"),
    (re.compile(r"^音频 · "), "音频 · <环境相关>"),
    (re.compile(r"^视觉 · "), "视觉 · <环境相关>"),
    (re.compile(r"^项目 · "), "项目 · <环境相关>"),
