...
REVERTS = [
    ...
    Revert(
        ...
        # ⚠⚠ 这条断点的锚点换过**两次**，两次都值得记：
        # ① 原来锚 `MAX_IDLE_RISK = 77` —— **开源验收门当场逮住**：
        #    子集里那个数是 **76**（测试文件少几支）⇒ 锚点不存在、断点失效。
        #    ⭐⭐ **锚在数字上的断点，在派生子集里必然失效 ——
        #      因为那个数正是子集要改的东西。**
        # ② 于是改锚那一行 `assert len(every) <= MAX_IDLE_RISK, (`，把它放宽 ——
        #    **手工验证时它没红**：棘轮有两条断言（不许涨 / 必须收紧），
        #    而实际值正好等于上限时，放宽上限那条什么都测不出来。
        #    ⭐⭐ **一个「把上限调大」的破坏，在「实际值正好等于上限」时是空的** ——
        #      棘轮的两条断言互为对方的兜底。
...
Revert(
    ...
    Revert(
        # 批 115：null 防护收进共用的 grenade_counts（special 里原来那道 `or {}` 与它叠成两道、
        # 拆一道不红 ⇒ 删了那道，锚点跟过来）
        "SWEEP", "投掷物检测又挡不住 weapons 显式 null",
        "core/gsi/kill_attribution.py",
    ...
    # ============================================ 批 115（RN-683/684/685）：不是枪打的人头 / 哪一个是我
    Revert(
        "NOTGUN", "炸弹炸死的人头又当成连杀",
        "gsi_handler_kills.py",
        "                    if self.bomb_kill_filter.is_bomb_kill(\n",
        "                    if False and self.bomb_kill_filter.is_bomb_kill(\n",
        "tests/test_kills_that_are_not_gunfire.py::test_bomb_kills_give_no_kill_feedback",
        "下的包炸死两个 CT，round_kills 一包跳两个 ⇒ 一枪没开却播三杀、弹三杀图标",
    ),
    Revert(
        "NOTGUN", "连杀等级没扣掉炸弹记走的人头",
        "gsi_handler_kills.py",
        "                        own_kills = current_round_kills - self.bomb_kill_offset.get(steamid, 0)\n",
        "                        own_kills = current_round_kills\n",
        "tests/test_kills_that_are_not_gunfire.py::test_a_real_kill_after_the_bomb_counts_from_the_real_number",
        "只拦了爆炸那一包 ⇒ 回合末补一枪按 round_kills 播成「四杀」",
    ),
    Revert(
        "NOTGUN", "炸弹过滤器按「现在是已爆」而不是「刚变成已爆」",
        "core/gsi/kill_attribution.py",
        '        if state == "exploded" and self._previous_state != "exploded":\n',
        '        if state == "exploded":\n',
        "tests/test_kills_that_are_not_gunfire.py::test_the_bomb_filter_only_counts_the_moment_it_went_off",
        "回合结束后 round.bomb 一直停在 exploded ⇒ 那段时间拿枪补的真击杀全被吞",
    ),
    Revert(
        "NOTGUN", "炸弹过滤器不看本帧开没开火",
        "core/gsi/kill_attribution.py",
        "        if fired or self._exploded_at is None:\n",
        "        if self._exploded_at is None:\n",
        "tests/test_kills_that_are_not_gunfire.py::test_gunfire_in_the_explosion_frame_is_still_a_kill",
        "爆炸那一包里真开枪打死的人也被当成炸弹的",
    ),
    Revert(
        "NOTGUN", "HUD 没接炸弹过滤器",
        "core/hud/runtime_engine.py",
        ...
        "            bomb_kill=False,\n",
        "tests/test_kills_that_are_not_gunfire.py::test_the_hud_does_not_flash_kill_colours_for_bomb_kills",
        "只修击杀音效那一处 ⇒ 本机看着修好了，HUD 还在闪连杀色（报告点名的「半修好」）",
    ),
    Revert(
        "NOTGUN", "雷杀又记成手里那把枪",
        "gsi_handler_kills.py",
        ...
        '        grenade = ""\n',
        "tests/test_kills_that_are_not_gunfire.py::test_a_grenade_kill_after_switching_back_belongs_to_the_grenade",
        "扔雷切 AK，雷炸死人 ⇒ AK 的专属击杀音在没开枪时响（社区：「投掷物击杀触发 21 冠音效」）",
    ),
    Revert(
        "NOTGUN", "开过枪也不清投掷记录",
        "gsi_handler_kills.py",
        ...
        "            pass\n",
        "tests/test_kills_that_are_not_gunfire.py::test_firing_a_gun_after_the_throw_gives_the_kill_back_to_the_gun",
        "扔完雷拿枪打死人也记给雷 ⇒ 过度归属，宁漏不错那条口径破了",
    ),
    Revert(
        "NOTGUN", "爆头也能算雷杀",
        "core/gsi/kill_attribution.py",
        ...
        "tests/test_kills_that_are_not_gunfire.py::test_a_headshot_is_never_the_grenade",
        "爆头只可能是枪 —— 丢了这条反证就把枪杀记给了雷",
    ),
    Revert(
        "NOTGUN", "投掷记录永不过期",
        "core/gsi/kill_attribution.py",
        ...
        "        if False:\n",
        "tests/test_kills_that_are_not_gunfire.py::test_a_kill_long_after_the_throw_is_not_the_grenade",
        "扔过一颗雷之后这一回合没开枪的击杀全记给雷（刀杀、电击枪杀）",
    ),
    Revert(
        "NOTGUN", "投掷音效的检测没走共用判定",
        "gsi_handler_special.py",
        "        thrown = thrown_grenade(\n",
        "        thrown = '' and thrown_grenade(\n",
        "tests/test_kills_that_are_not_gunfire.py::test_the_throw_sound_still_fires_through_the_shared_detector",
        "抽共用函数时漏接一头 ⇒ 投掷音效静默消失，而既有判据只直调 _play_grenade_sound，看不见",
    ),
    Revert(
        "NOTGUN", "「我是谁」又先信 config 里记的那个",
        "core/gsi/identity.py",
        '    return provider_steamid(data) or str(fallback or "").strip()\n',
        '    return str(fallback or "").strip() or provider_steamid(data)\n',
        "tests/test_kills_that_are_not_gunfire.py::test_provider_beats_a_wrongly_remembered_steamid",
        "第一帧在观战时记成了别人 ⇒ 观战静音反过来静掉自己的击杀",
    ),
    Revert(
        "NOTGUN", "第一次记 steamid 记的是 player 块",
        "gsi_handler_kills.py",
    ...
    Revert(
        ...
        "            pass\n",
        "tests/test_kill_icon_packs_keep_what_they_carry.py::test_the_author_and_license_survive_an_import_export_round_trip",
        "CS2 Customizer 又成了署名剥离器：别人的包经 CS2 Customizer 导出一次，作者和 license.txt 就没了",
    ),
    Revert(
        "CARRY", "导出不带随包文件",
        "core/kill_icon_pack.py",
    ...
    Revert(
        ...
        "        if bad is not None:\n",
        "        if False:\n",
        "tests/test_kill_icon_packs_keep_what_they_carry.py::test_a_broken_archive_never_replaces_the_old_file",
        "打出一个坏包还把用户原来那个好的顶掉",
    ),
    Revert(
        "CARRY", "拖进多个 zip 又只装第一个",
        "pages/kill_icon_page.py",
        "                for path in packs:\n",
        "                for path in packs[:1]:\n",
        "tests/test_kill_icon_packs_keep_what_they_carry.py::test_every_dragged_zip_is_imported_and_each_one_is_reported",
        "第 2 个起既不导入也不提示（报告点名的静默失败）",
    ),
    Revert(
        "CARRY", "混在 zip 里的单个素材又被静默丢掉",
        "pages/kill_icon_page.py",
        '            if result.get("skipped_assets"):\n',
        "            if False:\n",
        "tests/test_kill_icon_packs_keep_what_they_carry.py::test_every_dragged_zip_is_imported_and_each_one_is_reported",
        "有 zip 就整批走装包那条路，拖进来的 gif 一声不吭地没了",
    ),
    Revert(
        "CARRY", "导入探测缺 cols 又按 1 列",
        "core/kill_icon_import.py",
        "    cols = sheet_columns(data.get(\"cols\"), width, _as_int(data.get(\"frame_width\")))\n",
        "    cols = _as_int(data.get(\"cols\")) or 1\n",
        "tests/test_kill_icon_packs_keep_what_they_carry.py::test_a_missing_cols_is_read_the_same_way_on_import_as_at_runtime",
        "同一份文件两条路两个答案：运行时 3 列、导入 1 列（只读得出第一帧）",
    ),
    Revert(
        "CARRY", "原样搬运不设帧数上限",
        "core/kill_icon_import.py",
        ...
        "",
        "tests/test_kill_icon_packs_keep_what_they_carry.py::test_the_verbatim_path_also_caps_frames_and_writes_cols",
        "外来 JSON 写 9999 帧就原样进库 —— 只有这条路绕开了 600 帧闸门",
    ),
    Revert(
        "CARRY", "缩放又不看内存",
        "kill_icon_overlay.py",
        "    if need <= budget:\n",
        "    if True:\n",
        "tests/test_kill_icon_packs_keep_what_they_carry.py::test_an_oversized_style_is_shrunk_as_a_whole_to_fit_the_budget",
        "默认风格 200% ≈845MB 常驻；帧多画格大的外来素材能吃掉几个 GB",
    ),
    Revert(
        "CARRY", "逐帧目录又不量帧尺寸",
        "kill_icon_overlay.py",
        ...
        "            pass\n",
        "tests/test_kill_icon_packs_keep_what_they_carry.py::test_legacy_frame_folders_count_toward_the_budget",
        "内存上限对默认风格（正是逐帧目录）完全不起作用 —— 上限写了、对最常见的那套素材空转",
    ),
    Revert(
        "CARRY", "压了缩放不告诉用户",
        "pages/kill_icon_page.py",
        ...
        "        if False:\n",
        "tests/test_kill_icon_packs_keep_what_they_carry.py::test_the_page_says_when_the_scale_is_capped",
        "滑条拉到 200% 游戏里却是 154% —— 不说一声就是「滑条坏了」",
    ),
    # ============================================ 批 118：击杀图标可调
    Revert(
        "ADJUST", "切走前不存旧风格的位置",
        ...
        "            pass\n",
        "tests/test_kill_icon_is_adjustable_per_style.py::test_switching_styles_brings_back_each_ones_own_position",
        "甲从没调过、在乙上调了位置，切回甲拿到的是乙的位置",
    ),
    Revert(
        "ADJUST", "换风格不换位置",
        "pages/kill_icon_page.py",
        ...
        "        config.save_config()\n",
        "tests/test_kill_icon_is_adjustable_per_style.py::test_switching_styles_brings_back_each_ones_own_position",
        "位置只有一套全局值：换一套素材就得重调，换回去再调一遍（报告 §4 #3）",
    ),
    Revert(
        "ADJUST", "没记过的风格位置归零",
        "core/kill_icon_prefs.py",
        "    base = dict(fallback)\n",
        '    base = {"x": 0, "y": 0, "scale": 1.0}\n',
        "tests/test_kill_icon_is_adjustable_per_style.py::test_a_style_without_a_layout_inherits_the_current_one",
        "升级之后老用户调好的位置在第一次换风格时被清零",
    ),
    Revert(
        "ADJUST", "风格条又是 listdir 顺序",
        "core/kill_icon_prefs.py",
        "    rest = sorted(s for s in visible if s not in rank)\n",
        "    rest = [s for s in visible if s not in rank]\n",
        "tests/test_kill_icon_is_adjustable_per_style.py::test_recent_styles_come_first_and_the_rest_are_sorted",
        "装十几套之后常用的那套可能排在最后",
    ),
    Revert(
        "ADJUST", "能把正在用的那套也藏掉",
        ...
        "    if not style:\n",
        "tests/test_kill_icon_is_adjustable_per_style.py::test_you_cannot_hide_your_way_out_of_every_style",
        "藏掉当前风格 ⇒ 风格条上没有选中项，用户找不回来",
    ),
    Revert(
        "ADJUST", "藏到一套都不剩",
        "core/kill_icon_prefs.py",
        ...
        "    return True\n",
        "tests/test_kill_icon_is_adjustable_per_style.py::test_you_cannot_hide_your_way_out_of_every_style",
        "把自己锁死（对标对象那条「至少留一个可见」的兜底）",
    ),
    Revert(
        "ADJUST", "不透明度没乘进叠加层",
        "kill_icon_player.py",
        "    return float(fade_opacity) * user\n",
        "    return float(fade_opacity)\n",
        "tests/test_kill_icon_is_adjustable_per_style.py::test_the_overlay_multiplies_the_fade_by_the_user_opacity",
        "滑条和预览都变了，游戏里一点没变",
    ),
    Revert(
        "ADJUST", "不透明度不夹下限",
        ...
        '            self.kill_icon_opacity = min(1.0, float(data.get("kill_icon_opacity", 1.0)))\n',
        "tests/test_kill_icon_is_adjustable_per_style.py::test_garbage_in_config_does_not_break_loading",
        "手改成 0 ⇒ 图标彻底看不见，而页面上看起来一切正常",
    ),
    Revert(
        "ADJUST", "拖滑条不再约下一发试播",
        "pages/kill_icon_page.py",
        ...
        "        pass\n\n    def _replay_tick(self):\n",
        "tests/test_kill_icon_is_adjustable_per_style.py::test_adjusting_the_position_replays_only_when_idle_and_only_on_this_page",
        "调位置又回到「播完就没了，得再点一次」",
    ),
    Revert(
        "ADJUST", "重播不等上一发播完",
    ...
    Revert(
        ...
        "        if False:\n",
        "tests/test_kill_sounds_vary_and_voices_take_turns.py::test_numbered_files_become_variants_of_their_level",
        "3-2 / 3-3 白放，一局响几十次的击杀音还是同一条复读",
    ),
    Revert(
        "KSOUND", "只有一条也登记成变体",
        "core/audio/audio_manager.py",
        "            if len(variants) > 1:\n",
        "            if True:\n",
        "tests/test_kill_sounds_vary_and_voices_take_turns.py::test_a_level_with_one_file_has_no_variants",
        "单条也进变体表 —— 随机逻辑白跑一遍，且清账时多一份要清的",
    ),
    Revert(
        "KSOUND", "语音又绕开策略层",
        "core/audio/audio_manager.py",
        ...
        "        if False:\n",
        "tests/test_kill_sounds_vary_and_voices_take_turns.py::test_a_new_kill_voice_goes_through_the_policy_and_the_timeline",
        "新一杀顶不掉上一杀的语音；kill_voice=95 那一档继续没人用",
    ),
    Revert(
        "KSOUND", "语音播了不进时间线",
        "core/audio/audio_manager.py",
        '                action="play", key=key, channel_type="kill_voice", event_type="kill_voice",\n',
        '                action="play_hidden", key=key, channel_type="kill_voice", event_type="kill_voice",\n',
        "tests/test_kill_sounds_vary_and_voices_take_turns.py::test_a_new_kill_voice_goes_through_the_policy_and_the_timeline",
        "回放页上看不见语音发生过 —— 排「为什么没播报」时少一块证据",
    ),
    Revert(
        "KSOUND", "淡入淡出又直接取配置音量",
        "core/audio/audio_manager.py",
        "        target = self._resolve_play_volume(config, channel_type, info)\n",
        '        target = getattr(config, "round_sound_volume", self._volume)\n',
        "tests/test_kill_sounds_vary_and_voices_take_turns.py::test_the_fade_path_applies_loudness_normalization",
        "回合音效（唯一走淡入淡出）不吃响度归一：一套大声一套小声",
    ),
    Revert(
        "KSOUND", "助攻首帧就判定",
        "gsi_handler_special.py",
        ...
        "tests/test_kill_sounds_vary_and_voices_take_turns.py::test_an_assist_plays_once_and_the_first_frame_only_seeds",
        "中途启动软件：第一帧读到的 4 次助攻被当成一次新助攻（MVP 当年同一个坑）",
    ),
    Revert(
        "KSOUND", "助攻播种跟着总开关走",
        "gsi_handler_special.py",
        "        current = self._read_own_match_stat(data, \"assists\")\n",
        "        current = self._read_own_match_stat(data, \"assists\") if getattr(config, \"round_sound_enabled\", False) else None\n",
        "tests/test_kill_sounds_vary_and_voices_take_turns.py::test_assists_counted_while_the_switch_was_off_do_not_fire_later",
        "关着时计数照涨却没人记 ⇒ 一打开开关就补播一声",
    ),
    Revert(
        "KSOUND", "击杀图标页提示条按钮又压成 26px",
        "pages/kill_icon_page.py",
        ...
        "RN-697：「知道了」字被切成碎片、按钮横着撑满一整条（外审 S3 6/6）",
    ),
    Revert(
        "KSOUND", "共用提示条按钮又压成 26px",
        "widgets/page_notice_bar.py",
    ...
    Revert(
        "OSSSYNC", "行尾之差又算上游改动",
        "build_tools/oss_sync/sync_state.py",
        '    return a.replace(b"\\r\\n", b"\\n") == b.replace(b"\\r\\n", b"\\n")\n',
        "    return a == b\n",
        "tests/test_the_open_source_copy_does_not_fall_behind.py::test_line_endings_alone_are_not_an_upstream_change",
        "verdict.ps1 这种只差 CRLF 的文件又常驻在「上游也改了」那张单上",
    ),
    Revert(
        "OSSSYNC", "审过一次就永远不报",
        "build_tools/oss_sync/sync_state.py",
        "        (quiet if reviewed.get(rel) and reviewed.get(rel) == fingerprints.get(rel) else fresh).append(rel)\n",
        "        (quiet if rel in reviewed else fresh).append(rel)\n",
        "tests/test_the_open_source_copy_does_not_fall_behind.py::test_owned_files_are_quiet_until_upstream_changes_again",
        "关于页审过一次之后上游再改（批 116 那种诊断块）也不报 —— 比常驻噪音更糟",
    ),
    Revert(
        "OSSSYNC", "落后量把排除掉的也算上",
        "build_tools/oss_sync/sync_state.py",
        "        if line and is_public_path(line, exclude_prefix, generated):\n",
        "        if line:\n",
        "tests/test_the_open_source_copy_does_not_fall_behind.py::test_lag_counts_only_commits_that_touch_the_public_subset",
        "改一行服务器代码也报「开源仓落后」—— 狼来了，真落后时没人看",
    ),
    Revert(
        "OSSSYNC", "目录规则吞掉同前缀文件",
        "build_tools/oss_sync/sync_state.py",
        ...
        "tests/test_the_open_source_copy_does_not_fall_behind.py::test_a_directory_rule_does_not_swallow_a_same_prefix_file",
        "`server/` 把 `serverless.py` 也当成排除 ⇒ 真改动不计入落后量",
    ),
    Revert(
        "OSSSYNC", "同步完不记同步点",
        "build_tools/oss_sync/sync.py",
        "            sync_state.save_state(sync_state.mark_synced(sync_state.load_state(), head))\n",
        "            pass\n",
        "tests/test_the_open_source_copy_does_not_fall_behind.py::test_a_successful_gated_apply_records_the_sync_point",
        "同步点停在旧提交 ⇒ 全量总结行永远报「落后」，报久了就没人信",
    ),
    Revert(
        "OSSSYNC", "CHANGELOG 欠账报 0",
        "build_tools/oss_sync/sync.py",
        "    return int(count) if count.isdigit() else 0\n",
        "    return 0\n",
        "tests/test_the_open_source_copy_does_not_fall_behind.py::test_changelog_behind_counts_commits_since_it_last_changed",
        "09-19 之后 8 次同步没写 CHANGELOG 的那种欠账又变回静默",
    ),
    Revert(
        "OSSSYNC", "放行一切脏改动",
        "build_tools/oss_sync/sync_state.py",
        "    return \"\\n\".join(ln for ln in lines if ln[3:].strip().strip('\"') != \"CHANGELOG.md\")\n",
        "    return \"\"\n",
        "tests/test_the_open_source_copy_does_not_fall_behind.py::test_a_written_changelog_does_not_block_the_sync_but_other_dirt_does",
        "开源仓里没提交的手改被同步落盘盖掉",
    ),
    # ============================================ 批 120：视频直接变击杀图标
    Revert(
        "VIDEO", "长视频不截",
        "core/kill_icon_video.py",
        "    if seconds > MAX_SECONDS:\n",
        "    if False:\n",
        "tests/test_a_video_becomes_a_kill_icon.py::test_a_long_clip_is_cut_to_the_first_seconds",
        "拖进一段一分钟的视频就整段解码 —— 击杀图标一闪而过，却要等半天、图集撑到几百 MB",
    ),
    Revert(
        "VIDEO", "帧太多不降帧率直接砍",
        "core/kill_icon_video.py",
        "        if lower < fps:\n",
        "        if False:\n",
        "tests/test_a_video_becomes_a_kill_icon.py::test_too_many_frames_lowers_the_rate_before_decoding",
        "60 帧的 10 秒片子被砍成 5 秒 —— 用户要的整段动作只剩一半",
    ),
    Revert(
        "VIDEO", "取样只比帧起点",
        "core/kill_icon_video.py",
        '        while state["next"] < len(wanted) and wanted[state["next"]] < end:\n',
        '        while state["next"] < len(wanted) and wanted[state["next"]] <= frame.startTime():\n',
        "tests/test_a_video_becomes_a_kill_icon.py::test_a_higher_target_rate_holds_frames_through_the_last_one",
        "源帧率低于目标时片尾少一帧，还多报一句「视频比预计短」",
    ),
    Revert(
        "VIDEO", "帧率不看源",
        "core/kill_icon_video.py",
        ...
        "    return DEFAULT_FPS\n",
        "tests/test_a_video_becomes_a_kill_icon.py::test_a_video_is_probed_without_decoding_and_is_no_longer_refused",
        "20fps 的片子被拉成 30 帧/秒：同一帧重复、图集白大一半",
    ),
    Revert(
        "VIDEO", "解码出声",
        "core/kill_icon_video.py",
        ...
        "",
        "tests/test_a_video_becomes_a_kill_icon.py::test_decoding_never_makes_a_sound",
        "导入一段视频，音箱里把它 4 倍速放一遍",
    ),
    Revert(
        "VIDEO", "视频又被拒",
        "core/kill_icon_import.py",
        ...
        "    if False:\n",
        "tests/test_a_video_becomes_a_kill_icon.py::test_a_video_is_probed_without_decoding_and_is_no_longer_refused",
        "拖 mp4 进来又只得到一句「先去装 ffmpeg 转一下」",
    ),
    Revert(
        "VIDEO", "拖放不认视频",
        "widgets/kill_icon_level_grid.py",
        '".mp4", ".webm", ".mov", ".mkv",  # 批 120：视频\n',
        "  # 批 120：视频\n",
        "tests/test_a_video_becomes_a_kill_icon.py::test_the_video_extensions_are_accepted_at_every_door",
        "导入能解视频，拖放格子却不收 —— 用户最常用的那扇门是关着的",
    ),
    Revert(
        "VIDEO", "门牌上不写视频",
        "widgets/kill_icon_level_grid.py",
        ...
        "tests/test_a_video_becomes_a_kill_icon.py::test_the_video_extensions_are_accepted_at_every_door",
        "能拖视频，空格子上却只写 GIF / WebP / PNG —— 没人会去试",
    ),
    Revert(
        "VIDEO", "导入卡上不写视频",
        "widgets/kill_icon_style_strip.py",
        ...
        "tests/test_a_video_becomes_a_kill_icon.py::test_the_video_extensions_are_accepted_at_every_door",
        "风格条上最显眼的「＋ 导入」卡还写 zip / 动图 / 图片",
    ),
    Revert(
        "VIDEO", "打包又剔掉 QtMultimedia",
        "build_tools/build_release.py",
        '    "PySide6.QtMultimediaWidgets",\n',
        '    "PySide6.QtMultimedia",\n    "PySide6.QtMultimediaWidgets",\n',
        "tests/test_a_video_becomes_a_kill_icon.py::test_the_release_build_keeps_qt_multimedia",
        "源码里能导视频，装好的版本一导就报「打不开」",
    ),
    Revert(
        "VIDEO", "坏视频漏出内部异常",
        "core/kill_icon_import.py",
        ...
        "    duration, width, height, source_fps = probe_video(path)\n",
        "tests/test_a_video_becomes_a_kill_icon.py::test_a_broken_video_is_refused_in_plain_words",
        "拖进一个坏 mp4：页面只认 KillIconImportError，别的异常就是一个没人接的崩溃框",
    ),
]
